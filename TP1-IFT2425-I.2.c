...
/*FONCTION PRINCIPALE : la fonction du maximum de vraisemblance pour determiner c du maximum de vraisemblance*/
double func(double c, int N) {
    int i;
    double yPuissanceC;
    double ln;
    double num;
...
//implmentations du tableau contenant les valeurs de y
    double arrayY[] = {0.11, 0.24, 0.27, 0.52, 1.13, 1.54, 1.71, 1.84, 1.92, 2.01};


    //on calcule la premiere somme
    //facteur recupere la somme des facteurs entre y et lny
    facteur = 0;
    for (int i = 0; i < N-1; ++i) {
        facteur += facteur;
        ...
        yPuissanceC = pow(y,c);
        ln = log10(y);
        facteur = yPuissanceC * ln;
    }
    somme1 = facteur;

    //on calcule la deuxieme somme au denominateur de la fct
    for (int i = 0; i < N-1; ++i) {
        facteur2 += facteur2;
        ...
        yPuissanceC = pow(y,c);
        facteur2 = yPuissanceC ;
    }
    somme2 = facteur2;

    //on calcule la troisieme somme
    for (int i = 0; i < N-1; ++i) {
        facteur3 += facteur3;
    ...
    //on calcule ensuite les 3 quotients

    quotient1 = somme1/somme2;
    quotient2 = 1/c;
    quotient3 = somme3/N;

    //printf("%f", resultat);
    resultat = quotient1 - quotient2 - quotient3;
    //printf("%f", resultat);
...
/* 1a- Utiliser comme derivee une approximation du style f'(c)=(f(c+EPSILON)-f(c))/EPSILON*/
double derivee(double c1, int N) {

    double func1 = func(c1+EPSILON, N);
    double func2 = func(c1, N);

    return (func1 - func2)/EPSILON;
}

/* 1b -Utiliser comme derivee une approximation du style
 f'(c) =[ −f(x + 2 EPSILON) + 8 f(x + EPSILON) − 8 f(x − EPSILON) + f(x − 2 EPSILON)]/(12*EPSILON) */
double derivee2(double c, int N) {

    double func1 = func(c+2*EPSILON, N);
    double func2 = func(c+EPSILON, N);
    double func3 = func(c-EPSILON, N);
    double func4 = func(c-2*EPSILON, N);
...
{
    double h = func(x,10) / derivee2(x,10);
    while (abs(h) >= EPSILON)
    {
        h = func(x,10)/derivee2(x,10);
